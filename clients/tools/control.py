...
async def get_progress(prompt_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if not queue:
            return {"status": "error", "error": "Failed to get queue status"}
        ...
        if queue_running:
            ...
            if len(item) >= 2:
                current_job = {"prompt_id": item[1], "number": item[0]}
...
async def cancel(prompt_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        if prompt_id:
            ...
            if success:
                return {"status": "success", "message": f"Cancelled prompt {prompt_id}"}
            else:
                return {"status": "error", "error": f"Failed to cancel prompt {prompt_id}"}
        else:
            # Interrupt current generation
            success = _get_comfyui().interrupt()
            if success:
                return {"status": "success", "message": "Interrupted current generation"}
            else:
                return {"status": "error", "error": "Failed to interrupt current generation"}
    except Exception as e:
        return {"status": "error", "error": str(e)}
...
async def get_queue() -> Dict[str, Any]:
    ...
    try:
        ...
        for item in queue_running:
            if len(item) >= 2:
                running_jobs.append({"number": item[0], "prompt_id": item[1]})

        # Format pending jobs
        pending_jobs = []
        for item in queue_pending:
            if len(item) >= 2:
                pending_jobs.append({"number": item[0], "prompt_id": item[1]})
...
async def get_system_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if not _get_comfyui().check_availability():
            return {"status": "offline", "message": "ComfyUI server is not available"}
        ...
        system_info = {"status": "online", "system": stats.get("system", {}), "devices": stats.get("devices", [])}

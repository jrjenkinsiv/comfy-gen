...
def _get_model_registry():
    ...
    if _model_registry is None:
        from clients.models import ModelRegistry
...
async def generate_video(
    ...
    try:
        ...
        if not workflow:
            return {"status": "error", "error": "Failed to load Wan 2.2 T2V workflow"}

        # Apply parameters
        workflow = _get_workflow_mgr().set_prompt(workflow, prompt, negative_prompt)
        workflow = _get_workflow_mgr().set_seed(workflow, seed)
        workflow = _get_workflow_mgr().set_sampler_params(workflow, steps=steps, cfg=cfg)

        # Set video-specific parameters
        workflow = _get_workflow_mgr().set_video_params(workflow, width=width, height=height, length=frames)
        ...
        if not video_url:
            return {"status": "error", "error": "Failed to get output video URL", "prompt_id": prompt_id}
...
async def image_to_video(
    ...
    try:
        ...
        if not workflow:
            return {"status": "error", "error": "Failed to load Wan 2.2 I2V workflow"}
        ...
        workflow = _get_workflow_mgr().set_prompt(workflow, prompt, negative_prompt)
        workflow = _get_workflow_mgr().set_seed(workflow, seed)
        workflow = _get_workflow_mgr().set_sampler_params(workflow, steps=steps)

        # Set video-specific parameters
        workflow = _get_workflow_mgr().set_video_params(workflow, length=frames)
        workflow = _get_workflow_mgr().set_video_fps(workflow, fps=fps)
...
async def video_extend(
    ...
    Returns:
        Dictionary with status, url, and metadata
    """
    return {"status": "error", "error": "Video extension not yet implemented - requires custom workflow"}
...
async def interpolate_frames(
    ...
    Returns:
        Dictionary with status, url, and metadata
    """
    return {"status": "error", "error": "Frame interpolation not yet implemented - requires custom workflow"}

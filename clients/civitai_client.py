...
class CivitAIClient:
    ...
    def get_model_by_hash(self, file_hash: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            response = self.session.get(f"{self.BASE_URL}/model-versions/by-hash/{file_hash}", timeout=30)
            if response.status_code == 200:
                data = response.json()
                return {
    ...
    def get_download_url(self, model_id: int, version_id: Optional[int] = None) -> Optional[str]:
    ...
    def download_model(self, download_url: str, output_path: str, progress_callback: Optional[callable] = None) -> bool:

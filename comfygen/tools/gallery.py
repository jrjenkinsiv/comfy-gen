...
async def list_images(
    ...
    try:
        ...
        return {"status": "success", "images": images, "count": len(images), "total_in_bucket": len(objects)}
...
async def get_image_info(image_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not obj_info:
            return {"status": "error", "error": f"Image not found: {image_name}"}
        ...
        if history:
            ...
            for _prompt_id, prompt_data in history.items():
...
async def delete_image(image_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if success:
            return {"status": "success", "message": f"Deleted {image_name}"}
        else:
            return {"status": "error", "error": f"Failed to delete {image_name}"}
    except Exception as e:
        return {"status": "error", "error": str(e)}
...
async def get_history(limit: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        for prompt_id, prompt_data in history.items():
            ...
            history_list.append(
                {"prompt_id": prompt_id, "outputs": output_files, "status": status_info, "parameters": params}
            )
        ...
        return {"status": "success", "history": history_list, "count": len(history_list)}
    except Exception as e:
        return {"status": "error", "error": str(e)}

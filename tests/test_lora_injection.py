...
def test_load_lora_presets_success():
    ...
    mock_catalog = {
        "loras": [{"filename": "test_lora.safetensors", "recommended_strength": 0.8}],
        "model_suggestions": {"test_preset": {"default_loras": ["test_lora.safetensors"]}},
    }
...
def test_list_available_loras():
    ...
    mock_models = {"loras": ["lora1.safetensors", "lora2.safetensors"]}
...
def test_inject_lora_sd15_workflow():
    ...
    modified, new_id = generate.inject_lora(workflow, "test_lora.safetensors", strength_model=0.8, strength_clip=0.8)
...
def test_inject_lora_wan22_workflow():
    ...
    workflow = {
        "1": {"class_type": "UNETLoader", "inputs": {"unet_name": "wan2.2.safetensors"}},
        "2": {"class_type": "DualCLIPLoader", "inputs": {}},
        "3": {"class_type": "KSampler", "inputs": {"model": ["1", 0]}},
    }

    modified, new_id = generate.inject_lora(workflow, "wan_lora.safetensors", strength_model=1.0, strength_clip=1.0)
...
def test_inject_lora_chain():
    ...
    lora_specs = [("lora1.safetensors", 0.7, 0.7), ("lora2.safetensors", 0.5, 0.5)]
    ...
    # Count LoRA nodes
    lora_nodes = [node_id for node_id, node in modified.items() if node.get("class_type") == "LoraLoader"]
...
def test_inject_lora_chain_missing_lora():
    ...
    lora_specs = [("missing_lora.safetensors", 0.8, 0.8)]
...
def test_inject_lora_non_numeric_keys():
    ...
    workflow = {"abc": {"class_type": "CheckpointLoaderSimple", "inputs": {}}}

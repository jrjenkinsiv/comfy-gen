...
class TestCivitAIClient:
    ...
    def test_search_models_with_filters(self, mock_get):
        ...
        client = CivitAIClient()
        client.search_models(query="test", model_type="LORA", base_model="SD 1.5", sort="Highest Rated", nsfw=False)
    ...
    def test_get_model_by_id_success(self, mock_get):
        ...
        mock_response.json.return_value = {
            ...
            "creator": {"username": "testuser"},
            "modelVersions": [
                {"id": 1, "name": "v1.0", "baseModel": "SD 1.5", "files": [{"name": "model.safetensors"}]}
            ],
        }
        mock_get.return_value = mock_response
    ...
    def test_get_model_version_by_hash_success(self, mock_get):
        ...
        mock_response.json.return_value = {
            ...
            "name": "v1.0",
            "baseModel": "SD 1.5",
            "files": [{"name": "model.safetensors", "downloadUrl": "https://civitai.com/download/..."}],
    ...
    def test_get_download_url_success(self, mock_get):
        ...
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "files": [{"name": "model.safetensors", "downloadUrl": "https://civitai.com/download/123"}]
        }
        mock_get.return_value = mock_response
    ...
    def test_get_download_url_with_type_filter(self, mock_get):
        ...
        mock_response.json.return_value = {
            "files": [
                {"name": "model.ckpt", "type": "Model", "downloadUrl": "https://civitai.com/download/1"},
                {"name": "model.safetensors", "type": "Model", "downloadUrl": "https://civitai.com/download/2"},
            ]
        }
        mock_get.return_value = mock_response

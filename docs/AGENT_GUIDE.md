...
### Image Generation with LoRAs (Recommended)
...
LoRAs modify the model to achieve specific styles or effects. **You no longer need to manually edit workflow JSON files** - use the `--lora` CLI argument instead!

### Dynamic LoRA Injection (Recommended)

The easiest way to use LoRAs is via CLI arguments:
...
# Multiple LoRAs (automatically chained)
python3 generate.py \
    --workflow workflows/flux-dev.json \
    --prompt "..." \
...
See [LORA_INJECTION.md](LORA_INJECTION.md) for complete documentation.

### LoraLoader Node Parameters

If you need to manually edit workflow JSON, here are the node parameters:

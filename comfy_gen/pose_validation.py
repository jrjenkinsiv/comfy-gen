...
def crop_person_region(image: np.ndarray, bbox: list[float], padding_ratio: float = CROP_PADDING_RATIO) -> tuple[np.ndarray, tuple[int, int, int, int]]:
...
def estimate_pose_single(image: np.ndarray, visibility_threshold: float = DEFAULT_VISIBILITY_THRESHOLD) -> dict[str, Any]:
    ...
    if landmarker is None:
        return {
            'detected': False,
            'error': 'Failed to initialize MediaPipe pose detector'
        }

        ...
        if expected['subject_type'] == 'person':
            ...
            if answer:
                ...
                checks['subject_count'] = {
                    ...
                    'answer': answer,
                    'passed': detected_count is None or detected_count == expected['subject_count'] or (expected['subject_count'] == -1 and detected_count and detected_count > 1)
                }
                
                if not checks['subject_count']['passed']:
                    issues.append(f"Subject count mismatch: expected {expected['subject_count']}, detected {detected_count}")
